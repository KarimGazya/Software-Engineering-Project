...
for (let i = 0; i < temp.organizations.length; i++) {
  ...
  card.className = "organizaton-card card";
  card.id = `${i}`;
...
let organizationCard = document.querySelectorAll(".organizaton-card");

organizationCard.forEach((o) => {
  o.addEventListener("click", (e) => {
    document.querySelector(".organization-select").style.display = "block";
    if (id === null) {
    ...
    <div class="container heading-selected">
        <div class="row">
        <div class="col-md-8"><button class="btn btn-sm delete-btn text">Delete</button><button class="btn btn-sm delete-btn text">Delete</button><button class="btn btn-sm delete-btn text">Delete</button></div>
          </div>
      </div>
    <p class="text">
